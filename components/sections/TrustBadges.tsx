...
export function TrustBadges() {
  return (
    <section className="py-8 md:py-12 bg-white border-y border-brand-100">
      <Container size="xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          {BADGES.map((badge) => (
            <div
              key={badge.value}
              className="flex flex-col items-center text-center p-3 md:p-4 rounded-card bg-brand-50/50 border border-brand-100 hover:border-brand-300 hover:shadow-soft transition"
            >
              <div className="text-2xl md:text-3xl mb-1.5">{badge.icon}</div>
              <p className="text-[10px] md:text-xs text-gray-500 mb-0.5">
                {badge.label}
              </p>
              <p className="font-display text-xs md:text-sm font-bold text-brand-900 leading-tight">
                {badge.value}
              </p>
            </div>
          ))}
